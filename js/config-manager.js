...
const CONFIG_SCHEMA = {
  ...
  rooms: {
    predefined: {
      type: 'array',
      default: [
        { name: 'Bath 1', icon: '🛁', category: 'bathroom' },
        { name: 'Bath 2', icon: '🛁', category: 'bathroom' },
        { name: 'Bath 3', icon: '🛁', category: 'bathroom' },
        { name: 'Ensuite', icon: '🚿', category: 'bathroom' },
        { name: 'Powder', icon: '🚽', category: 'bathroom' },
        { name: 'Kitchen', icon: '🍽️', category: 'kitchen' },
        { name: 'Butlers', icon: '👨‍🍳', category: 'kitchen' },
        { name: 'Laundry', icon: '🧺', category: 'utility' },
        { name: 'Alfresco', icon: '🍽️', category: 'outdoor' }
      ]
    },
...
export class ConfigManager {
   ...
   */
    ...
    for (const [key, value] of Object.entries(schema)) {
      if (value && typeof value === 'object' && value.type === 'object' && !value.default) {
        // This is a nested object definition, recurse
        config[key] = this.buildDefaultConfig(value);
      } else if (value && typeof value === 'object' && !value.type && !value.default) {
        // This is a nested schema section (like 'app', 'api', etc.)
        config[key] = this.buildDefaultConfig(value);
      } else if (value && value.default !== undefined) {
        // This is a config value with a default
   ...
   */
    ...
    if (!this.config.pdf) this.config.pdf = {};
